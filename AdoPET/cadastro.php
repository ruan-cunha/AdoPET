...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nome) || empty($email) || empty($senha) || empty($confirm_senha) || empty($tipo_usuario)) {
        set_flash_message('Todos os campos obrigat�rios devem ser preenchidos.', 'danger');
    } elseif ($senha !== $confirm_senha) {
        set_flash_message('As senhas n�o coincidem.', 'danger');
    } elseif (strlen($senha) < 6) {
        set_flash_message('A senha deve ter no m�nimo 6 caracteres.', 'danger');
    } else {
        ...
        if ($result->num_rows > 0) {
            set_flash_message('Este email j� est� cadastrado.', 'danger');
        } else {
            ...
            if ($stmt->execute()) {
                set_flash_message('Cadastro realizado com sucesso! Fa�a login para continuar.', 'success');
...
<section class="form-section">
    ...
    <form method="POST" action="cadastro.php" onsubmit="return validarCadastro()">
        ...
        <small class="help-text">M�nimo de 6 caracteres.</small>
        ...
        <select id="tipo_usuario" name="tipo_usuario" onchange="toggleCamposCadastro()" required>
            ...
            <option value="Pessoa Fisica" <?php echo (($form_data['tipo_usuario'] ?? '') == 'Pessoa Fisica') ? 'selected' : ''; ?>>Pessoa F�sica</option>
        ...
        <div id="campos_ong" class="form-group hidden">
            ...
            <label for="descricao_ong">Descri��o da ONG:</label>
