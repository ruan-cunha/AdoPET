...
if (!isset($_SESSION['user_id'])) {
    set_flash_message('Fa�a login para manifestar interesse.', 'warning');
...
// Checar se j� existe um interesse ativo
...
if ($stmt->get_result()->num_rows > 0) {
    set_flash_message('Voc� j� tem um interesse ativo neste animal. Verifique seu painel.', 'info');
    header('Location: animal_detalhes.php?id=' . $animal_id);
    exit();
}
$stmt->close();
...
if ($stmt->execute()) {
    set_flash_message('Seu interesse foi registrado com sucesso! O doador ser� notificado.', 'success');
} else {
